    ...
    return (
      <li key={node.id}>
        ...
        >
          ...
          <span className="flex w-5 shrink-0 items-center justify-center text-[14px] leading-none" aria-hidden="true">
            {node.icon ? (
              node.icon
            ) : node.type === 'task' ? (
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-zinc-200/80 bg-[#f7f7f5] text-[14px]">
      ...
      <div className="px-2">
        ...
        <NavLink href="/tasks" label="업무 찾기" icon="✅" active={pathname.startsWith('/tasks')} />
        <NavLink href="/team" label="진행 상황" icon="👥" active={pathname.startsWith('/team')} />
...
function LitePageLink({ p, active }: { p: SidebarPageLite; active: boolean }) {
  return (
    ...
    >
      <span className="flex w-5 shrink-0 items-center justify-center text-[14px] leading-none">{p.icon ?? (p.type === 'task' ? '✅' : '📄')}</span>
...
function NavLink({ href, label, icon, active }: { href: string; label: string; icon: string; active: boolean }) {
  return (
    ...
    >
      <span className="w-5 text-center text-[14px] leading-none" aria-hidden="true">
        {icon}
      </span>
      {label}
    </Link>
