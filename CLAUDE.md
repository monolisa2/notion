...
- **다크 모드 색은 `app/globals.css` 의 매핑 표가 정한다** (`.dark .bg-zinc-200 { … }` 식).
  이 표는 **CSS 레이어 밖**이라 Tailwind 의 `dark:` 유틸리티를 **항상 이긴다**.
  그래서 `bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900` 처럼 **다크에서 색을 뒤집는
  조합은 무조건 깨진다** (실제로 버튼 14개가 대비 1.7:1 로 안 보였다).
  주요 동작 버튼·선택된 토글은 `className="btn-primary"` 를 쓸 것 — 색은 `--btn-bg`/`--btn-fg` 한 곳에서.
  새 색이 필요하면 `dark:` 를 덧붙이지 말고 매핑 표에 한 줄을 추가하거나 토큰을 만들 것
...
- **페이지 화면 순서는 제목 → 업무 속성·진행 상황 → 본문 → 하위 페이지 → 진행 기록 → 댓글.**
  제목·본문·자동저장이 `PageEditor` 하나에 묶여 있어 제목만 떼어낼 수 없으므로,
  속성 줄·진행률 패널은 `PageEditor` 의 `afterTitle` 슬롯으로 넘긴다.
  **이 위에 무언가를 더 얹지 말 것** — 전에 속성·진행률·하위 페이지가 전부 제목 위에 있어서
  페이지를 열면 제목이 한 화면 아래로 밀려 무슨 페이지인지가 안 보였다
